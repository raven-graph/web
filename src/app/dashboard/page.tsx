...
const MarketNetwork: React.FC = () => {
  ...
  const nodes = [
    // Tech Stocks
    { ticker: 'AAPL', sector: 'tech', color: '#B066FF', x: 25, y: 25, size: 10, riskScore: 4, sentiment: 'bullish', influences: ['SPY', 'VIX'] },
    { ticker: 'NVDA', sector: 'tech', color: '#B066FF', x: 35, y: 15, size: 10, riskScore: 3, sentiment: 'bullish', influences: ['AAPL', 'MSFT'] },
    { ticker: 'MSFT', sector: 'tech', color: '#B066FF', x: 15, y: 35, size: 10, riskScore: 6, sentiment: 'neutral', influences: ['SPY'] },
    { ticker: 'GOOGL', sector: 'tech', color: '#B066FF', x: 45, y: 25, size: 10, riskScore: 5, sentiment: 'bullish', influences: ['AAPL', 'NVDA'] },
    
    // Finance Stocks
    { ticker: 'JPM', sector: 'finance', color: '#4ECDC4', x: 65, y: 30, size: 10, riskScore: 7, sentiment: 'bearish', influences: ['DXY', 'SPY'] },
    { ticker: 'BAC', sector: 'finance', color: '#4ECDC4', x: 75, y: 20, size: 10, riskScore: 6, sentiment: 'neutral', influences: ['JPM', 'WFC'] },
    { ticker: 'GS', sector: 'finance', color: '#4ECDC4', x: 55, y: 40, size: 10, riskScore: 8, sentiment: 'bearish', influences: ['JPM', 'DXY'] },
    { ticker: 'WFC', sector: 'finance', color: '#4ECDC4', x: 85, y: 35, size: 10, riskScore: 5, sentiment: 'neutral', influences: ['BAC'] },
    
    // Insurance Stocks
    { ticker: 'BRK.B', sector: 'insurance', color: '#45B7D1', x: 20, y: 65, size: 10, riskScore: 3, sentiment: 'bullish', influences: ['SPY', 'AAPL'] },
    { ticker: 'AXP', sector: 'insurance', color: '#45B7D1', x: 30, y: 75, size: 10, riskScore: 7, sentiment: 'bearish', influences: ['JPM'] },
    { ticker: 'AIG', sector: 'insurance', color: '#45B7D1', x: 10, y: 55, size: 10, riskScore: 6, sentiment: 'neutral', influences: ['DXY', 'VIX'] },
    
    // Energy Stocks
    { ticker: 'XOM', sector: 'energy', color: '#FF6B6B', x: 70, y: 70, size: 10, riskScore: 9, sentiment: 'bearish', influences: ['CVX', 'COP'] },
    { ticker: 'CVX', sector: 'energy', color: '#FF6B6B', x: 80, y: 60, size: 10, riskScore: 8, sentiment: 'bearish', influences: ['XOM', 'DXY'] },
    { ticker: 'COP', sector: 'energy', color: '#FF6B6B', x: 60, y: 80, size: 10, riskScore: 7, sentiment: 'neutral', influences: ['XOM'] },
    
    // Indices
    { ticker: 'SPY', sector: 'index', color: '#96CEB4', x: 50, y: 50, size: 12, riskScore: 4, sentiment: 'bullish', influences: [] },
    { ticker: 'DXY', sector: 'index', color: '#96CEB4', x: 40, y: 60, size: 12, riskScore: 6, sentiment: 'neutral', influences: [] },
    { ticker: 'VIX', sector: 'index', color: '#96CEB4', x: 60, y: 50, size: 12, riskScore: 9, sentiment: 'bearish', influences: [] }
  ];
    ...
    <div className="relative w-full h-96 bg-zinc-900/60 rounded-2xl border border-white/20 p-4 overflow-hidden">
      <svg viewBox="0 0 100 100" className="w-full h-full">
        ...
        <g stroke="#6B7280" strokeWidth="0.4" opacity="0.6">
          {nodes.map((node, i) => 
            nodes.slice(i + 1).map((other, j) => {
              // Create more realistic connections (not all nodes connected to all)
              const shouldConnect = Math.random() > 0.25; // 75% chance of connection
        ...
        {nodes.map((node, i) => {
          ...
          const info = getNodeInfo(node.ticker);
          ...
          return (
            <g key={node.ticker}>
              ...
              <circle
                ...
                onClick={() => setSelectedNode(isSelected ? null : node.ticker)}
              >
                <animate
                  attributeName="r"
...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-zinc-950 via-zinc-950 to-zinc-900 text-zinc-100">
      {/* Header */}
      <header className="sticky top-0 z-50 backdrop-blur supports-[backdrop-filter]:bg-zinc-900/40 border-b border-white/10">
